...
namespace GlobalAI.FileDomain.Implements
{
    public class FileServices : IFileServices
    {
        ...
        private string GetImageNameToSave(string fileName)
        {
            ...
            return $"globalai-{Guid.NewGuid().ToString("N")}{ext}";
        }
        ...
        private byte[] GetFile(string folder, string fileName, FileConfig config)
        {
            ...
            {
                throw new FaultException(new FaultReason($"Không cho phép định dạng file này"), new FaultCode(((int)ErrorCode.FileExtensionNoAllow).ToString()), "");
            }
